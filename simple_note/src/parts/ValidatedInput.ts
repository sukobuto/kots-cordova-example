...
function extractField(data, name):KnockoutObservable<any> {
	data = ko.unwrap(data);
	return ko.isObservable(data[name])
		? data[name]
		: ko.getObservable(data, name);
}
...
ko.components.register('select-input', {
	...
	viewModel: {
		createViewModel (params) {
			...
			vm['field'] = extractField(params.data, params.field_name);
			vm['field'].subscribe(newValue => console.log(newValue));
			return vm;
		}
	}
});
